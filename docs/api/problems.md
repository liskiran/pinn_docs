...
$$ u_t + u_{xx} = e^{-\gamma^2t} \cdot (-\gamma^2 \cdot (a \cdot \cos{\alpha x} + b \cdot \sin{beta \cdot x}) + b \cdot \beta^2 * \sin{beta \cdot x} + a \cdot \alpha^2 \cdot \cos{\alpha \cdot x})$$
...
$$ u_t + u_{xx} = 2 - 4 \cdot e^{2x}, x \in [0, 2], y \in [0, 1] $$
