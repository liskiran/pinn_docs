...
Класс, позволяющий создать сетку точек в указанной области. Созданные данные можно использовать для построения тепловых карт и расчета значения функций на структурированном наборе точек
...
- **save_mode** (str) : режим сохранения данных. В комлпексе представлены режимы:
...
- **min** (float | None) : минимальное значение colorbar.
- **max** (float | None) : максимальное значение colorbar.
...
> **_Важно:_** Данную функцию следует вызывает отдельно от других callback
...
- **metric_history** (list) : список для сохранения истории значений метрик

**Методы**

- **draw(self, values: Sequence, v_names: [str], coord: np.ndarray, plot_title: str)** : Строит и сохраняет график.
- **dict_data(fig)** : сохраняет данные графика в виде словаря.
- **init_curve_names(self, conditions)** : назначает названия кривым в зависимости от условия задачи.
